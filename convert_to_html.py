...
def normalize(df, brand, cfg):
    ...
    # 리솜: 리조트명 분리 (타워 -> 제천 레스트리, 빌라 -> 제천 포레스트)
    if brand == "리솜":
        df["리조트명"] = [
            "제천 레스트리" if "타워" in str(rt) and "제천" in str(res)
            else "제천 포레스트" if ("빌라" in str(rt) or "포레스트" in str(rt)) and "제천" in str(res)
            else "제천 포레스트" if "제천" in str(res) # 기본값은 제천 포레스트
            else res
            for rt, res in zip(df["객실타입"], df["리조트명"])
        ]
        df["리조트명"] = [
            str(res).replace("레스트리 제천", "제천 레스트리")
            .replace("포레스트 제천", "제천 포레스트")
            .replace("스플라스 덕산", "덕산 스플라스")
            .replace("아일랜드 안면도", "안면도 아일랜드")
            for res in df["리조트명"]
        ]
    ...
    if brand == "롯데":
        def translate_lotte_room(name):
            name = str(name).strip()
            table = {
                # 영문 및 기본 객실타입 치환 (콘도/호텔/스페셜 접두어 추가)
                "Deluxe Double": "호텔 디럭스 더블",
                "Deluxe Family Twin": "호텔 디럭스 패밀리 트윈",
                "Grand Deluxe Family": "호텔 그랜드 디럭스 패밀리",
                "Junior Family Suite": "호텔 주니어 패밀리 스위트",
                "Superior Suite": "호텔 슈페리어 스위트",
                "Superior Suite With Tempur": "호텔 슈페리어 스위트 (템퍼)",
                "Luxury 45A": "스페셜 럭셔리 45평 A타입",
                "Luxury 45B": "스페셜 럭셔리 45평 B타입",
                "Suite 33D": "콘도 스위트 33평 D타입",
                "Suite 33T": "콘도 스위트 33평 T타입",
                "Family 23": "콘도 패밀리 23평",
                "LOTTY&LORRY 23": "캐릭터 패밀리 로티&로리",
                "LOTTY&LORRY 33": "캐릭터 스위트 로티&로리",
                "Deluxe 18F": "콘도 디럭스 18평 F타입",
                "Deluxe 18H": "콘도 디럭스 18평 H타입",
                "Deluxe Double(H)": "콘도 디럭스 더블 배리어프리",
                "DLH": "콘도 디럭스 더블 배리어프리",
                ...
                # 평형별 상세명칭 (콘도 접두어 추가)
                "18 A- TYPE 온돌": "콘도 18평 A타입 온돌",
                "18 A- TYPE 트윈": "콘도 18평 A타입 트윈",
                "18 C-TYPE 온돌 트윈": "콘도 18평 C타입 온돌트윈",
                "23 A-TYPE 더블+온돌": "콘도 23평 A타입 더블+온돌",
                "23 C-TYPE 온돌 더블+온돌": "콘도 23평 C타입 더블+온돌",
                "23 C-TYPE 온돌 싱글더블": "콘도 23평 C타입 싱글더블",
                "31 A-TYPE 더블+온돌온돌": "콘도 31평 A타입 더블+온돌",
                "31 A-TYPE 트윈+온돌": "콘도 31평 A타입 트윈+온돌",
                "31 A-TYPE 트윈+온돌온돌": "콘도 31평 A타입 트윈+온돌",
                "31 B-TYPE 더블+온돌온돌": "콘도 31평 B타입 더블+온돌",
                "31 B-TYPE 트윈+온돌온돌": "콘도 31평 B타입 트윈+온돌",
                "31 C-TYPE 온돌 더블+온돌온돌": "콘도 31평 C타입 더블+온돌",
                "31 C-TYPE 온돌 트윈+온돌온돌": "콘도 31평 C타입 트윈+온돌",
                "31 C-TYPE 풀 더블+온돌": "콘도 31평 C타입 풀빌라 더블+온돌",
                "31 C-TYPE 키즈 온돌": "콘도 31평 C타입 키즈 온돌",
                "31 C-TYPE 키즈 패밀리": "콘도 31평 C타입 키즈 패밀리",
                "45 B-TYPE 더블+더블온돌": "콘도 45평 B타입 더블+더블+온돌",
                "45 B-TYPE 트윈+더블온돌": "콘도 45평 B타입 트윈+더블+온돌",
            }
            if name in table:
                return table[name]
            ...
            if transformed.startswith("C ") or transformed.startswith("C"):
                transformed = "콘도 " + transformed[1:].strip()
            elif transformed.startswith("H ") or transformed.startswith("H"):
                transformed = "호텔 " + transformed[1:].strip()
